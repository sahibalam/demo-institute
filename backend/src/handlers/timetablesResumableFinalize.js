...
export async function handler(event) {
  ...
  try {
    ...
    try {
      ...
      console.error('timetablesResumableFinalize permissions error', e);
    }
    ...
    const fileName = sanitize(body.fileName) || sanitize(meta?.data?.name) || `${klass}-${section}-timetable.pdf`;
    ...
    console.error('timetablesResumableFinalize error', e);
